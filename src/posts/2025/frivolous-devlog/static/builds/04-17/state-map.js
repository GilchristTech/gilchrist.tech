...
export class MapState extends State {
  ...
  onPush () {
    ...
    if (this.tile_map) {
      ...
      while (this.tile_map.circleCollidesSolid(spawn_x, center_y, this.player.radius)) {
  ...
  onEntityKillsOther (entity, other) {
    if (entity === this.player) {
      if (this.kill_requirement != null && this.player.character.num_killed >= this.kill_requirement) {
        this.setResult("win");
      }
    }
  }
...
export class WinState extends ElementState {
  ...
  onPush () {
    ...
    const num_killed = this.map_state?.player.character.num_killed;
...
export class MapInterfaceState extends ElementState {
  ...
  onDraw (tick, time) {
    ...
    if (this?.map_state.player) {
      this.kill_counter_value_element.textContent = `${ this.map_state.player.character.num_killed }`;
