...
export class Enemy extends Entity {
  init (name="enemy", character=null) {
    character ||= enemy_characters[0].copy();
    ...
    this.spritesheet = this.character.spritesheet || "monsters";
    this.sprite_i    = this.character.sprite_i    || 0;

    this.target = null;

    this.target_bias = Math.PI * (
        (2*Math.random()-1) *
        (character.target_bias_spread - character.target_bias_min) +
        character.target_bias_min
      );
  ...
  onTick (tick) {
    ...
    const target_distance = Math.sqrt(rise * rise + run * run) - (target.radius || 0);
    ...
    this.x += this.dx * step_distance;
    this.y += this.dy * step_distance;
  }
  ...
  recalculateDirection (rise, run, target_distance) {
    ...
    //
    const bias_theta = this.target_bias * (1 - Math.pow(2, Math.pow(2*bias_coefficient, 0.5)));
