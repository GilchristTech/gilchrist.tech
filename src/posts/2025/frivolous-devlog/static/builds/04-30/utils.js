...
export function shuffleArrayInplace (array) {
  for (let pivot=0; pivot < array.length; pivot++) {
    const swap_index = Math.random() * (array.length - pivot) >> 0;
    [array[swap_index], array[pivot]] = [array[pivot], array[swap_index]];
  }
  return array;
}


export function chooseRandomFrom (list, num=null) {
  if (num === null) {
    return list[Math.random() * list.length >> 0];
  } else {
    return shuffleArrayInplace([...list]).slice(-num);
  }
}
