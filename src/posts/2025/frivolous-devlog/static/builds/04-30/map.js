...
import spritesheets from "./assets.js";

export const
  TILE_IS_SOLID    = 0o1_000,
  TILE_HAS_SHADOW  = 0o2_000,
  TILE_IS_ENTRANCE = 0o3_000,
  TILE_IS_EXIT     = 0o4_000;

export const TILE_IS_WALL = TILE_IS_SOLID | TILE_HAS_SHADOW;

export const
  TILE_WALL     = TILE_IS_WALL | 0o001,
  TILE_ENTRANCE = TILE_IS_WALL | TILE_IS_ENTRANCE | 0o002,
  TILE_EXIT     = TILE_IS_EXIT | TILE_IS_SOLID    | 0o003;
...
export class EntityMap {
  ...
  spawnEntity (entity, x, y) {
    if (!(entity instanceof Entity)) {
      throw "Entity in arguments is not an Entity";
    }
  ...
  assertEntity (entity) {
    ...
    if (!entity.row_end && !entity.row_next && !entity.row_prev) {
      throw "Entity row data is undefined";
    }
  }
  ...
  assertNotEntity (entity) {
    ...
    throw "Assertion error, map entity is valid";
  }
...
export class TileMap {
  constructor (obj={}) {
    ...
    this.default_tile = obj.default_tile || (TILE_IS_WALL|1);
    ...
    if (obj.rows) {
      ...
      this.tiles_w = obj.tiles_w || 25;
      this.tiles_h = obj.tiles_h || 25;

      this.rows = [];

      this.setTiles(null, 0, 0, this.tiles_w, this.tiles_h);
    }
    ...
    this.tile_renderer_index = [
      ...
      (ctx, sx, sy, dx, dy, scale) => {
        ...
        const down_tile = (down_row?.[sx] & this.tile_mask) == 0o001;
      ...
      // Entrance
      (ctx, sx, sy, dx, dy, scale) => {
        const sheet = spritesheets.tiles;
        const size = this.tile_size * scale;
        const y_offset = size/4;
        sheet.drawCell(ctx, 8, 16, dx, dy-y_offset, size, size+y_offset);
      },
      ...
      (ctx, sx, sy, dx, dy, scale) => {
        ...
        const size = this.tile_size * scale;
        const y_offset = size/4;
        sheet.drawCell(ctx, 7, 16, dx, dy-y_offset, size, size+y_offset);
      },
    ];
  }
  ...
  getTileSolid (x, y) {
    ...
    if (tile_code == null)
      return false;
    return (tile_code & TILE_IS_SOLID) == TILE_IS_SOLID;
  }

  circleCollidesMask (x, y, radius, mask) {
    ...
    for (let tile_x = left; tile_x <= right;  tile_x += tile_size) {
      if (
        ((this.getTile(tile_x, tile_y) & mask) == mask) &&
  ...
  circleCollidesSolid (x, y, radius) {
    return this.circleCollidesMask(x, y, radius, TILE_IS_SOLID);
  }
  ...
  */
  generateDivision (wall, tx, ty, tw, th, generation={}) {
    ...
    const wall_min_length = generation.wall_min_length ?? 2;
  ...
  generateDivisionMaze (wall, tx, ty, tw, th, generation={}) {
    ...
    const level = generation.level ?? null;

    generation.wall_thickness  ??= 2;
    generation.hall_thickness  ??= 3;
    generation.vertical        ??= false;
    generation.room_min_size   ??= 1;
    generation.wall_min_length ??= 2;
    ...
    if (level > 0 || level === null) {
      for (let quadrant of regions.quadrants) {
        quadrant.regions = this.generateDivisionMaze(
          wall, quadrant.x, quadrant.y, quadrant.w, quadrant.h, {
            ...generation,
